...
class EditScreen(ModalScreen[Task]):
    ...
    def __init__(self, task_id: str, list_name: str, name: str | None = None, id: str | None = None, classes: str | None = None) -> None:
    ...
    async def on_mount(self) -> None:
        ...
        if not self._task:
            self.app.notify(f"Task with ID {self._task_id} not found.", title="Error", severity="error")
            self.dismiss()
            return
        ...
        self.query_one("#due-date-label", Label).update(str(self._task.due_date) if self._task.due_date else "No due date")

    def compose(self) -> ComposeResult:
        with Vertical(id="edit-task-dialog") as container:
            container.border_title = "Edit Task"
            yield Label("Description:")
            yield Input(value="", id="description") # Initial empty value, will be updated in on_mount
            yield Label("Tags (comma-separated):")
            yield Input(value="", id="tags") # Initial empty value, will be updated in on_mount
            yield Label("Priority:")
            yield Input(value="", id="priority") # Initial empty value, will be updated in on_mount
            yield Label("Flagged:")
            yield Switch(value=False, id="flagged") # Initial empty value, will be updated in on_mount
            yield Label("Due Date:")
            yield Label("No due date", id="due-date-label") # Initial empty value, will be updated in on_mount
